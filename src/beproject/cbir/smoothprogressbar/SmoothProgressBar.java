package beproject.cbir.smoothprogressbar;
...
public class SmoothProgressBar extends ProgressBar {
	...
	public SmoothProgressBar(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.SmoothProgressBar, defStyle, 0);
		...
		final int width = a.getDimensionPixelSize(
				...
				res.getDimensionPixelSize(R.dimen.spb_default_stroke_width));
		final String strSpeed = a
				.getString(R.styleable.SmoothProgressBar_spb_speed);
		final int iInterpolator = a.getInteger(
				R.styleable.SmoothProgressBar_spb_interpolator,
		...
		final int colorsId = a.getResourceId(
				R.styleable.SmoothProgressBar_spb_colors, 0);
		a.recycle();
		...
		SmoothProgressDrawable.Builder builder = new SmoothProgressDrawable.Builder(
				...
				.width(width).reversed(reversed).mirrorMode(mirrorMode);

		if (strSpeed != null)
			builder.speed(Float.parseFloat(strSpeed));
